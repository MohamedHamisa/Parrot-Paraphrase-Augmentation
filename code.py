...
#beam-search multinomial sampling by calling beam_sample() if num_beams>1 and do_sample=True
            para_phrases.sort(key=lambda x:x[1], reverse=True) #Desc
            return para_phrases[0]
        else:
            return [(save_phrase,0)]
# method for improving NLP models for matching incoming data with stored data in large database 
#by leveraging edit distance/Levenshtein and couting the required number of changes to transform this 
